...
def main(det_callback, tb_callback, model_args, data_args, training_args):
    ...
    num_train_epochs=1
    t_total = len(train_dataset) // gradient_accumulation_steps * num_train_epochs
    optimizer_grouped_parameters = [
        {'params': [p for n, p in trainer.model.named_parameters() if not any(nd in n for nd in no_decay)], 'weight_decay': weight_decay},
        {'params': [p for n, p in trainer.model.named_parameters() if any(nd in n for nd in no_decay)], 'weight_decay': 0.0}
        ]
    trainer.optimizer = AdamW(optimizer_grouped_parameters, lr=5e-5, eps=1e-5)
    trainer.lr_scheduler = get_linear_schedule_with_warmup(trainer.optimizer, num_warmup_steps=warmup_steps, num_training_steps=t_total)
    print("trainer.optimizer: ",trainer.optimizer)
    print("trainer.lr_scheduler: ",trainer.lr_scheduler)
    # trainer.lr_scheduler = LambdaLR(trainer.optimizer, 1, last_epoch=-1)
...
#     hparams = info.trial.hparams
    hparams = {
          ...
          'output_dir' '/cstor/mendeza/hf_output/':True,
...
#         )
    ...
    main(None, None, model_args, data_args, training_args)